...
Tab:AddButton({
    ...
    Callback = function()
        -- Atualiza a lista de nomes dos jogadores
        local novosJogadores = {}
        for _, player in ipairs(Players:GetPlayers()) do
            table.insert(novosJogadores, player.Name)
        end

        -- Limpa a variável de seleção
        Target = nil

        -- Atualiza o dropdown (limpa a seleção anterior)
        Dropdown:Refresh(novosJogadores, true)

        -- Força visualmente o reset do texto do dropdown
        -- Isso depende da biblioteca. Tenta os dois:
        if Dropdown.Set then
            Dropdown:Set(nil) -- Reseta sem valor selecionado
        end
        if Dropdown.Text then
            Dropdown.Text = "Selecionar Jogador" -- (opcional)
        end
    end
})
